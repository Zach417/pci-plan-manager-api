...
userSetupRequestSchema.methods.generateTokenAndSendEmail = function(callback) {
    ...
    createToken(function (tokenValue) {
        ...
        var html = EmailSender.Emails.UserSetupRequest
            .replace(new RegExp("__LINK__", 'g'),"http://ec2-52-37-62-183.us-west-2.compute.amazonaws.com/register/"+this._id+"?token="+tokenValue)
            .replace("__NAME__", this.firstName);
        var emailSender = new EmailSender({
            from: "zallen@pension-consultants.com",
            to: this.email,
            subject: "PlanManager - Email Verification",
            html: html,
        });
        emailSender.send();
        callback();
